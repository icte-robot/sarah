...
export default function App() {
  ...
  if (!isListening) {
    return (
      <div className="flex flex-col h-screen font-sans bg-[#F5F5F0] items-center justify-center text-center p-6">
        <div className="scale-150 mb-12">
          <SarahFace emotion="happy" isTalking={false} />
        </div>
        <h1 className="text-5xl font-serif text-[#5A5A40] font-bold mt-8">Hi, I'm Sarah!</h1>
        <p className="text-xl text-gray-600 mt-4 mb-12 max-w-md">I'm so excited to talk to you! Click the button below to start.</p>
        
        {micError && (
          <div className="mb-8 p-4 bg-red-50 border border-red-200 text-red-600 rounded-xl max-w-md animate-bounce">
            {micError}
          </div>
        )}
        ...
        <button 
          ...
          className="px-12 py-6 bg-[#5A5A40] text-white rounded-full font-bold text-2xl flex items-center gap-4 hover:bg-opacity-90 transition-all duration-300 shadow-xl active:scale-95 mb-4"
        >
          <Mic size={32}/>
          Start Talking
        </button>
  ...
  return (
    <div className="flex flex-col h-screen font-sans bg-[#F5F5F0] overflow-hidden">
      {/* Immersive Sarah Display Area - 2/3 of the screen */}
      <div className="h-[66vh] bg-white/95 backdrop-blur-2xl shadow-lg border-b border-gray-200 flex flex-col items-center justify-center relative transition-all duration-500">
        {/* Audio Visualizer Ring */}
        <div 
          className="absolute w-64 h-64 rounded-full border-4 border-[#5A5A40]/20 transition-transform duration-75"
          style={{ 
        ...
        <div className="absolute bottom-8 text-center">
          {isLoading && (
            <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-[#5A5A40] text-white px-4 py-1 rounded-full text-sm font-bold animate-pulse">
              I'm thinking...
            </div>
          )}
          {micLevel > 20 && !isTalking && !isLoading && (
            <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-[#5A5A40] text-white px-4 py-1 rounded-full text-sm font-bold animate-bounce">
              I'm listening!
            </div>
          )}
          <h1 className="text-4xl font-serif text-[#5A5A40] font-bold tracking-tight">Sarah</h1>
          <div className="flex items-center justify-center gap-3 mt-2">
            <div className={`w-4 h-4 rounded-full shadow-sm ${isListening ? (micLevel > 10 ? 'bg-green-500 animate-ping' : 'bg-green-500 animate-pulse') : 'bg-red-500'}`}></div>
            <span className="text-sm font-black text-gray-400 uppercase tracking-[0.2em]">{conversationState}</span>
          </div>
          {isListening && (
            <div className="mt-4 w-32 h-1 bg-gray-100 rounded-full overflow-hidden mx-auto">
              <div 
                className="h-full bg-[#5A5A40] transition-all duration-75"
        ...
        <div className="absolute top-4 right-6 flex items-center gap-4">
           <button 
             ...
             className="px-3 py-1 bg-white/50 hover:bg-white text-[#5A5A40] rounded-md text-[10px] font-bold transition-colors border border-[#5A5A40]/10"
           >
             STOP LESSONS
           </button>
           <button 
             ...
             className="px-3 py-1 bg-gray-100 hover:bg-gray-200 text-gray-500 rounded-md text-[10px] font-bold transition-colors"
           >
             RESET SESSION
           </button>
           <span className="text-[10px] text-gray-300 font-mono font-bold">7" IMMERSIVE MODE</span>
        </div>
      </div>

      {/* Conversation Area - 1/3 of the screen - ONLY Spanish text */}
      <main ref={chatContainerRef} className="h-[34vh] overflow-y-auto p-8 space-y-6 bg-white/50 relative">
        {conversationState === 'AWAITING_PRONUNCIATION' && (
          <div className="sticky top-0 z-10 flex justify-center mb-4">
            <button 
              ...
              className="px-6 py-2 bg-white border-2 border-[#5A5A40]/20 text-[#5A5A40] rounded-full text-sm font-bold shadow-md hover:bg-gray-50 transition-all"
            >
        ...
        {messages.map((msg, index) => {
          ...
          return (
            <div key={index} className={`flex items-end gap-4 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[90%] px-10 py-6 rounded-[3rem] text-3xl shadow-sm ${
                msg.sender === 'user' 
                  ? 'bg-[#5A5A40] text-white rounded-br-none shadow-xl' 
                  : 'bg-white text-[#5A5A40] rounded-bl-none border-2 border-[#5A5A40]/10 font-bold italic'
              }`}>
                <div className="prose prose-2xl max-w-none prose-p:my-0 leading-relaxed">
                  {displayText}
                </div>
              </div>
            </div>
         ...
         {isLoading && (
            <div className="flex items-end gap-4 justify-start">
                <div className="max-w-lg px-8 py-5 rounded-[2.5rem] bg-white text-gray-800 rounded-bl-none shadow-sm border border-gray-100">
                    <div className="flex items-center justify-center space-x-3">
                        <div className="w-4 h-4 bg-[#5A5A40]/30 rounded-full animate-bounce [animation-delay:-0.3s]"></div>
                        <div className="w-4 h-4 bg-[#5A5A40]/30 rounded-full animate-bounce [animation-delay:-0.15s]"></div>
                        <div className="w-4 h-4 bg-[#5A5A40]/30 rounded-full animate-bounce"></div>
                    </div>
                </div>
            </div>
        )}
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center gap-2">
        ...
        <div className="px-6 py-2 rounded-full bg-white/80 backdrop-blur-md border border-gray-200 shadow-lg flex items-center gap-3">
           ...
           <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">{getPlaceholder()}</span>
        </div>
      </div>
    </div>
  );
